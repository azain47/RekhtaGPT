...
class GPT2(nn.Module):
    def __init__(self, config:GPTConfig) -> None:
        ...
        self.transformer_wpe = None if config.use_rotary else nn.Embedding(config.block_size, config.n_embed)
        self.transformer_attn = nn.ModuleList([Block(config) for _ in range(config.n_layer)])
    ...
    def forward(self, idx, targets = None):
        ...
        for block in self.transformer_attn:
            x = block(x)
